...
func (c *Client) SlotsInfo() (map[int]int, error) {
	// 获取当前client的所有slot信息，由于我们这一步还没有给group分配槽，所以这里得到的reply是一个len和cap都为0的interface切片
	// 会向redis发送一个"slotsinfo"命令，此命令是codis-server单独添加的一个命令，普通redis服务器并不支持此命令，dashboard根据此命令返回值来判断可否将此redis添加到codis-group中
	if reply, err := c.Do("SLOTSINFO"); err != nil {
		return nil, errors.Trace(err)
	} else {
		// 这里的info也是len和cap都为0的interface切片
...
func NewPool(auth string, timeout time.Duration) *Pool {
	...
	if timeout != 0 {
		go func() {
			...
			for {
				select {
				// 连接池收到退出的消息，就直接return
				case <-p.exit.C:
					return
				case <-ticker.C: // 每隔1分钟清理Pool中无效的Client
					p.Cleanup()
				}
			}
		}()
...
func (p *Pool) InfoFull(addr string) (_ map[string]string, err error) {
	...
	defer p.PutClient(c)
	return c.InfoFull() // 发送info命令和CONFIG GET MAXMEMORY命令获取全量信息
}
