...
func newApiServer(p *Proxy) http.Handler {
	...
	r.Group("/proxy", func(r martini.Router) {
		...
		r.Get("/readblacklist/get", api.GetReadBlackList)
		r.Put("/readblacklist/set/:addr", api.SetReadBlackList)
		r.Put("/readblacklist/del/:addr", api.DelReadBlackList)
		r.Get("/clientipblacklist/get", api.GetClientIpBlackList)
		r.Put("/clientipblacklist/set/:addr", api.SetClientIpBlackList)
		r.Put("/clientipblacklist/del/:addr", api.DelClientIpBlackList)
		r.Get("/cmdblacklist/get", api.GetCMDBlackList)
		r.Put("/cmdblacklist/set/:cmd/:key", api.SetCMDBlackList)
		r.Put("/cmdblacklist/del/:cmd/:key", api.DelCMDBlackList)
	})
...
func (s *apiServer) GetReadBlackList() (int, string) {
	return rpc.ApiResponseJson(s.proxy.GetReadBlackList())
}

func (s *apiServer) SetReadBlackList(params martini.Params) (int, string) {
	addr := params["addr"]
	addrs := strings.Split(addr, ",")
	check_addr_error := errors.New("readblacklist need redis addr:port or redis addr:port list use , separator.")
	if len(addrs) > 0 {
		for _, v := range addrs {
			if utils.CheckAddr(v) {
				s.proxy.SetReadBlackList(v)
			} else {
				log.Warnf("api SetReadBlackList fail , %v", check_addr_error)
				return rpc.ApiResponseError(check_addr_error)
			}
		}
	}
...
func (s *apiServer) DelReadBlackList(params martini.Params) (int, string) {
	addr := params["addr"]
	addrs := strings.Split(addr, ",")
	check_addr_error := errors.New("readblacklist need redis addr:port or redis addr:port list use , separator.")
	if len(addrs) > 0 {
		for _, v := range addrs {
			if utils.CheckAddr(v) {
				s.proxy.DelReadBlackList(v)
			} else {
				log.Warnf("api DelReadBlackList fail , %v", check_addr_error)
				return rpc.ApiResponseError(check_addr_error)
			}
		}
	}
...
func (s *apiServer) GetClientIpBlackList() (int, string) {
	return rpc.ApiResponseJson(s.proxy.GetClientIpBlackList())
}

func (s *apiServer) SetClientIpBlackList(params martini.Params) (int, string) {
	addr := params["addr"]
	addrs := strings.Split(addr, ",")
	check_addr_error := errors.New("client ip black need client ip addr or ip addr list use , separator.")
	if len(addrs) > 0 {
		for _, v := range addrs {
			if net.ParseIP(v) != nil {
				s.proxy.SetClientIpBlackList(v)
			} else {
				log.Warnf("api SetClientIpBlackList fail , %v", check_addr_error)
				return rpc.ApiResponseError(check_addr_error)
			}
		}
	}
...
func (s *apiServer) DelClientIpBlackList(params martini.Params) (int, string) {
	addr := params["addr"]
	addrs := strings.Split(addr, ",")
	check_addr_error := errors.New("client ip black need client ip addr or ip addr list use , separator.")
	if len(addrs) > 0 {
		for _, v := range addrs {
			if net.ParseIP(v) != nil {
				s.proxy.DelClientIpBlackList(v)
			} else {
				log.Warnf("api DelClientIpBlackList fail , %v", check_addr_error)
				return rpc.ApiResponseError(check_addr_error)
			}
		}
	}
...
func (s *apiServer) GetCMDBlackList() (int, string) {
	return rpc.ApiResponseJson(s.proxy.GetCMDBlackList())
}

func (s *apiServer) SetCMDBlackList(params martini.Params) (int, string) {
	cmd := params["cmd"]
	key := params["key"]
	check_params_error := errors.New("cmd and key can not be empty.")
	if len(cmd) > 0 && len(key) > 0 {
		cmd = strings.ToUpper(string(cmd))
		s.proxy.SetCMDBlackList(key, cmd)
	} else {
		log.Warnf("api SetCMDBlackList fail , %v", check_params_error)
		return rpc.ApiResponseError(check_params_error)
	}
	return rpc.ApiResponseJson("OK")
}

func (s *apiServer) DelCMDBlackList(params martini.Params) (int, string) {
	cmd := params["cmd"]
	key := params["key"]
	check_params_error := errors.New("cmd and key can not be empty.")
	if len(cmd) > 0 && len(key) > 0 {
		cmd = strings.ToUpper(string(cmd))
		s.proxy.DelCMDBlackList(key, cmd)
	} else {
		log.Warnf("api DelCMDBlackList fail , %v", check_params_error)
		return rpc.ApiResponseError(check_params_error)
	}
	return rpc.ApiResponseJson("OK")
}
